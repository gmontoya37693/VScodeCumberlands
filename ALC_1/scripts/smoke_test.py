...
def main() -> int:
    ...
    if args.api_key:
        ...
        status, body = _request(
            "GET",
            f"{base}/api/v1/portfolio/snapshot?as_of={as_of}",
            api_key=args.api_key,
        )
        _check(
            "portfolio/snapshot returns 200 with request_id, totals, and assets",
            status == 200 and "request_id" in body and "totals" in body and "assets" in body,
            results,
        )
