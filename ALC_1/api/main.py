...
import alc_item_sheet_tracker as engine
...
class DailyRunRequest(BaseModel):
    ...
    as_of: date
    billing_day: int = Field(default=31, ge=1, le=31)
...
def _run_response(result: Any, request_id: str, **details: str) -> dict[str, Any]:
    ...
    # Nothing after this point needs the run's scratch folder; clear it so
    # working/<run_id> doesn't accumulate across a busy burst of activity.
...
@app.get("/api/v1/portfolio/snapshot", dependencies=[Depends(require_api_key)])
def get_portfolio_snapshot(
    as_of: date, request_id: str = Depends(get_request_id)
) -> dict[str, Any]:
    """Read-only, in-process snapshot: no subprocess, no manifest, no working/ folder."""
    logger.info("request_id=%s endpoint=portfolio/snapshot as_of=%s", request_id, as_of.isoformat())
    try:
        assets = engine.load_assets(storage.inputs / "assets.csv")
        rates = engine.load_rates(storage.inputs / "rates.csv")
        posted_rows = engine.posted_invoice_map(
            engine.load_posted_invoices(storage.state / "posted_invoices.csv")
        )
    except (OSError, ValueError) as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
    rows = engine.build_snapshot(assets, rates, as_of, posted_rows=posted_rows)
    totals = engine.rows_total(rows)
    return {
        "request_id": request_id,
